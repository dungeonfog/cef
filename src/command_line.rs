use cef_sys::{cef_command_line_t, cef_command_line_get_global, cef_command_line_create, cef_string_userfree_utf16_free, cef_string_map_alloc, cef_string_map_size, cef_string_map_key, cef_string_map_value, cef_string_map_free};
use std::{
    ptr::null_mut,
    collections::HashMap,
};
...
impl CommandLine {
    ...
    pub fn new_from_argv(argv: &[&str]) -> Self {
        ...
        let argv: Vec<*const c_char> = argv.iter().map(|arg| CString::new(*arg).unwrap().as_ptr()).collect();

        unsafe { ((*instance).init_from_argv.unwrap())(instance, argv.len() as i32, argv.as_ptr()); }
        Self(instance)
    }
    ...
    pub fn new_from_string(command_line: &str) -> Self {
        let instance = unsafe { cef_command_line_create() };
        unsafe { ((*instance).init_from_string.unwrap())(instance, CefString::new(command_line).as_ref()); }
        Self(instance)
    }
    /// Reset the command-line switches and arguments but leave the program
    /// component unchanged.
    pub fn reset(&mut self) {
        unsafe { (self.as_ref().reset.unwrap())(self.0); }
    }
    /// Retrieve the original command line string as a vector of strings. The argv
    /// array: `{ program, [(--|-|/)switch[=value]]*, [--], [argument]* }`
    pub fn get_argv(&self) -> Vec<String> {
        let list = CefStringList::new();
        unsafe { (self.as_ref().get_argv.unwrap())(self.0, list.get()); }
        list.into()
    }
    ...
    pub fn get_command_line_string(&self) -> String {
        ...
        let command_line_str = String::from_utf16_lossy(unsafe { std::slice::from_raw_parts((*command_line).str, (*command_line).length) });

        unsafe { cef_string_userfree_utf16_free(command_line); }
        command_line_str
    }
    /// Get the program part of the command line string (the first item).
    pub fn get_program(&self) -> String {
        let program = unsafe { (self.as_ref().get_program.unwrap())(self.0) };
        let program_str = String::from_utf16_lossy(unsafe { std::slice::from_raw_parts((*program).str, (*program).length) });

        unsafe { cef_string_userfree_utf16_free(program); }
        program_str
    }
    /// Set the program part of the command line string (the first item).
    pub fn set_program(&mut self, program: &str) {
        let program = CefString::new(program);
        unsafe { (self.as_ref().set_program.unwrap())(self.0, program.as_ref()); }
    }
    ...
    pub fn get_switch_value(&self, name: &str) -> Option<String> {
        let value = unsafe { (self.as_ref().get_switch_value.unwrap())(self.0, CefString::new(name).as_ref()) };
        if value.is_null() {
            return None;
        }
        let value_str = String::from_utf16_lossy(unsafe { std::slice::from_raw_parts((*value).str, (*value).length) });

        unsafe { cef_string_userfree_utf16_free(value); }
        Some(value_str)
    }
    ...
    pub fn get_switches(&self) -> HashMap<String, Option<String>> {
        ...
        unsafe { (self.as_ref().get_switches.unwrap())(self.0, switches); }

        let result = (0..unsafe { cef_string_map_size(switches) }).map(|index| {
            let pair = (null_mut(), null_mut());
            unsafe {
                cef_string_map_key(switches, index, pair.0);
                cef_string_map_value(switches, index, pair.1);
            }
            (CefString::copy_raw_to_string(pair.0).unwrap(), CefString::copy_raw_to_string(pair.1))
        }).collect();

        unsafe { cef_string_map_free(switches); }
        result
    }
    /// Add a switch to the end of the command line.
    pub fn append_switch(&mut self, name: &str) {
        unsafe { (self.as_ref().append_switch.unwrap())(self.0, CefString::new(name).as_ref()); }
    }
    /// Add a switch with the specified value to the end of the command line.
    pub fn append_switch_with_value(&mut self, name: &str, value: &str) {
        unsafe { (self.as_ref().append_switch_with_value.unwrap())(self.0, CefString::new(name).as_ref(), CefString::new(value).as_ref()); }
    }
    ...
    pub fn get_arguments(&self) -> Vec<String>  {
        let list = CefStringList::new();
        unsafe { (self.as_ref().get_arguments.unwrap())(self.0, list.get()); }
        list.into()
    }
    /// Add an argument to the end of the command line.
    pub fn append_argument(&mut self, argument: &str) {
        unsafe { (self.as_ref().append_argument.unwrap())(self.0, CefString::new(argument).as_ref()); }
    }
    /// Insert a command before the current command. Common for debuggers, like
    /// "valgrind" or "`gdb --args`".
    pub fn prepend_wrapper(&mut self, wrapper: &str) {
        unsafe { (self.as_ref().prepend_wrapper.unwrap())(self.0, CefString::new(wrapper).as_ref()); }
    }
}
