use cef_sys::{cef_resource_bundle_handler_t, cef_string_userfree_utf16_alloc, cef_string_t, cef_string_utf8_to_utf16, cef_scale_factor_t, cef_string_utf16_set};
use std::{
    ptr::null_mut,
    sync::Arc,
    convert::TryFrom,
...
pub(crate) struct ResourceBundleHandlerWrapper(Box<dyn ResourceBundleHandler>, Box<cef_resource_bundle_handler_t>);

impl ResourceBundleHandlerWrapper {
    pub(crate) fn wrap(handler: Box<dyn ResourceBundleHandler>) -> Arc<Self> {
        let bundle = RefCounted::wrap(cef_resource_bundle_handler_t {
        ...
        let hash = Hashed::from(&*bundle);
        let instance = Arc::new(Self(handler, bundle));
        reference::register(hash, &instance);
        instance
    }
    ...
    extern "C" fn get_localized_string(self_: *mut cef_resource_bundle_handler_t, string_id: std::os::raw::c_int, string: *mut cef_string_t) -> std::os::raw::c_int {
        if let Ok(string_id) = PackString::try_from(string_id) {
            if let Some(this) = reference::get::<Self>(Hashed::from(self_)) {
                match this.0.get_localized_string(string_id, &CefString::copy_raw_to_string(string).unwrap()) {
                    None => 0,
                    Some(rstr) => {
    ...
    extern "C" fn get_data_resource(self_: *mut cef_resource_bundle_handler_t, resource_id: std::os::raw::c_int, data: *mut *mut std::os::raw::c_void, data_size: *mut usize) -> std::os::raw::c_int {
        if let Ok(resource_id) = PackResource::try_from(resource_id) {
            if let Some(this) = reference::get::<Self>(Hashed::from(self_)) {
                match this.0.get_data_resource(resource_id) {
                    None => 0,
                    Some(bytes) => {
                        unsafe {
                            (*data_size) = bytes.len();
    ...
    extern "C" fn get_data_resource_for_scale(self_: *mut cef_resource_bundle_handler_t, resource_id: std::os::raw::c_int, scale_factor: cef_scale_factor_t, data: *mut *mut std::os::raw::c_void, data_size: *mut usize) -> std::os::raw::c_int {
        if let Ok(resource_id) = PackResource::try_from(resource_id) {
            if let Some(this) = reference::get::<Self>(Hashed::from(self_)) {
                match this.0.get_data_resource_for_scale(resource_id, ScaleFactor::wrap(scale_factor)) {
                    None => 0,
                    Some(bytes) => {
                        unsafe {
                            (*data_size) = bytes.len();
...
impl Drop for ResourceBundleHandlerWrapper {
    fn drop(&mut self) {
        reference::unregister::<Self>(Hashed::from(&self.1));
    }
}
