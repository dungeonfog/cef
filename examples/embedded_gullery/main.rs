...
impl RenderHandlerCallbacks for RenderHandlerCallbacksImpl {
    fn get_view_rect(&self, _: Browser) -> Rect {
        let inner_size = self.blit.lock().window().inner_size();
        Rect {
            x: 0,
            y: 0,
    ...
    fn get_screen_point(
        ...
        let screen_pos = self.blit.lock().window().inner_position().unwrap_or(PhysicalPosition::new(0, 0));
        let point_physical = LogicalPosition::new(point.x, point.y).to_physical::<i32>(self.blit.lock().window().scale_factor());
        Some(Point::new(screen_pos.x + point_physical.x, screen_pos.y + point_physical.y))
    }
    fn on_popup_size(&self, _: Browser, mut rect: Rect) {
        let window_size: (u32, u32) = self.blit.lock().window().inner_size().into();
    ...
    fn get_screen_info(&self, _: Browser) -> Option<ScreenInfo> {
        ...
        let window = blit.window();
        let inner_size = window.inner_size().to_logical::<f64>(window.scale_factor());
        let rect = Rect {
            x: 0,
            y: 0,
    ...
    fn on_paint(
        ...
        let mut pixel_buffer = self.blit.lock();
    ...
    fn on_cursor_change(
        ...
        match winit_cursor {
            Some(cursor) => {
                self.blit.lock().window().set_cursor_icon(cursor);
                self.blit.lock().window().set_cursor_visible(true);
            },
            None => self.blit.lock().window().set_cursor_visible(false),
        }
    }
...
fn main() {
    match cef::process_type() {
        ...
        cef::ProcessType::Browser => {
            ...
            let width = blit.window().inner_size().width;
            let height = blit.window().inner_size().width;

            let window_info = WindowInfo {
                windowless_rendering_enabled: true,
                parent_window: Some(unsafe{ RawWindow::from_window(blit.window()) }),
                width: width as _,
                height: height as _,
                ..WindowInfo::new()
            };
