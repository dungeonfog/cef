...
mod winit_blit;
#[cfg(not(any(
    feature = "winit-blit-renderer",
    feature = "gullery-renderer",
...
use winit::{
    ...
    window::{CursorIcon, Icon, Window, WindowBuilder},
};
...
pub struct BrowserProcessHandlerCallbacksImpl {
    proxy: Mutex<EventLoopProxy<CefEvent>>,
    init_done: Mutex<Option<mpsc::Sender<()>>>,
}
...
impl BrowserProcessHandlerCallbacks for BrowserProcessHandlerCallbacksImpl {
    fn on_context_initialized(&self) {
        if let Some(init_done) = self.init_done.lock().take() {
    ...
    fn on_cursor_change(&self, _browser: Browser, _cursor: cef_cursor_handle_t, type_: CursorType) {
        ...
        let winit_cursor = match type_ {
            ...
            CursorType::Custom(custom_cursor) => {
                let hot_spot = PhysicalPosition::new(
                    custom_cursor.hotspot.x as u32,
                    custom_cursor.hotspot.y as u32,
                );
                let size = PhysicalSize::new(
                    custom_cursor.size.width as u32,
                    custom_cursor.size.height as u32,
                );
                ...
                Some(
                    Icon::from_rgba_with_hot_spot(&buffer, size, hot_spot)
                        .ok()
                        .map(|c| CursorIcon::Custom(c))
...
fn main() {
    match cef::process_type() {
        ...
        cef::ProcessType::Browser => {
            ...
            let window_builder = WindowBuilder::new().with_title("CEF Example Window");
            ...
            let (init_sender, init_receiver): (mpsc::Sender<()>, _) = mpsc::channel();
            let proxy = event_loop.create_proxy();
            let mut browser_thread = Some(std::thread::spawn(move || {
                let app = App::new(AppCallbacksImpl {
                    browser_process_handler: BrowserProcessHandler::new(
                        BrowserProcessHandlerCallbacksImpl {
                            proxy: Mutex::new(proxy),
                            init_done: Mutex::new(Some(init_sender)),
                        },
                    ),
                });
            ...
            let (browser_sender, browser_receiver): (mpsc::Sender<cef::browser::Browser>, _) =
                mpsc::channel();
            cef::task::TaskRunner::post_task_on(cef::task::ThreadId::UI, move || {
                let window_info = WindowInfo {
                    windowless_rendering_enabled: true,
                    parent_window: Some(unsafe {
                        RawWindow::from_window(renderer.lock().window())
                    }),
                    width: width as _,
                    height: height as _,
                    ..WindowInfo::new()
                ...
                browser_sender.send(browser).unwrap();
            });

            let browser = browser_receiver.recv().unwrap();
            ...
            event_loop.run(move |event, _, control_flow| {
                match event {
                    ...
                    Event::UserEvent(event) => match event {
                        CefEvent::Quit => {
                            if let Some(browser_thread) = browser_thread.take() {
                                cef::task::TaskRunner::post_task_on(
                                    cef::task::ThreadId::UI,
                                    move || {
